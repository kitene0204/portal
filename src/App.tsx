...
export default function App() {
  ...
  const [activeTab, setActiveTab] = useState<'school' | 'personal'>('school');
  ...
  const activeTheme = DEFAULT_THEMES[config.themeId] || DEFAULT_THEMES.indigo;
  const isDark = true; // All themes are dark/sleek cosmic geometric presets

  // Filters apps based on active category tab, search query, and sorts by order
  const filteredApps = apps
    .filter(app => {
      // If we are in Compact Split layout, we display school & personal side by side, 
      // so we don't apply the tab filter in that layout, only the search filter!
      const matchesCategory = config.layoutId === 'split' ? true : app.category === activeTab;
  ...
  return (
    <div className={`min-h-screen flex flex-col transition-colors duration-300 font-sans ${activeTheme.bg}`}>
      ...
      <main className="flex-1 max-w-7xl w-full mx-auto px-4 sm:px-6 lg:px-8 py-8">
        ...
        {/* Categories Tab Selector (Hidden in Compact Split view, because Split shows both side-by-side!) */}
        {config.layoutId !== 'split' && (
          <div className="flex justify-between items-center mb-8 border-b border-neutral-200/10 pb-4">
            <div className="flex gap-1.5 p-1 bg-neutral-950/40 border border-slate-800/40 rounded-xl">
              <button
        ...
        {filteredApps.length === 0 && (config.layoutId !== 'split' || (schoolAppsSplit.length === 0 && personalAppsSplit.length === 0)) ? (
              ...
              >
                {filteredApps.map((app) => (
                  <a
                    ...
                    className={`group p-6 rounded-2xl border transition-all duration-300 block relative hover:-translate-y-1.5 hover:shadow-xl hover:shadow-black/5 ${activeTheme.cardBg} ${activeTheme.border}`}
                  >
            ...
            {config.layoutId === 'list' && (
              ...
              >
                {filteredApps.map((app) => (
                  <div
                    key={app.id}
                    className={`p-5 rounded-xl border flex flex-col sm:flex-row sm:items-center justify-between gap-4 transition-all duration-300 hover:shadow-md ${activeTheme.cardBg} ${activeTheme.border}`}
                  >
                    <div className="flex items-start gap-4 flex-1 min-w-0">
                      <div className={`p-3 rounded-lg flex-shrink-0 mt-0.5 ${
                        app.category === 'school' 
                          ? isDark ? 'bg-sky-500/10 text-sky-400' : 'bg-sky-50 text-sky-700'
                          : isDark ? 'bg-violet-500/10 text-violet-400' : 'bg-violet-50 text-violet-700'
                      }`}>
              ...
              >
                ...
                {filteredApps.length > 0 && (
                  <div className="lg:col-span-7 flex flex-col">
                    <div className={`p-8 rounded-3xl border flex-1 flex flex-col justify-between transition-all duration-300 shadow-lg relative overflow-hidden group ${activeTheme.cardBg} ${activeTheme.border}`}>
                  ...
                  {filteredApps.slice(1).length === 0 ? (
                    ...
                    filteredApps.slice(1).map((app) => (
                      <a
                        ...
                        target="_blank"
                        rel="noopener noreferrer"
                        className={`p-4 rounded-xl border flex items-center justify-between gap-3 transition-all duration-300 hover:translate-x-1.5 ${activeTheme.cardBg} ${activeTheme.border}`}
                      >
                ...
                <div className="space-y-4">
                  <div className="flex items-center justify-between border-b border-neutral-200/10 pb-3">
                    <h3 className={`text-sm font-bold flex items-center gap-2 ${activeTheme.text}`}>
                      ...
                      학교 프로젝트 포털 ({schoolAppsSplit.length})
                    </h3>
                    ...
                    {schoolAppsSplit.length === 0 ? (
                      ...
                      schoolAppsSplit.map((app) => (
                        <a
                          ...
                          target="_blank"
                          rel="noopener noreferrer"
                          className={`p-4 rounded-xl border block transition-all duration-300 hover:shadow-md hover:border-sky-500/30 ${activeTheme.cardBg} ${activeTheme.border}`}
                        >
                ...
                <div className="space-y-4">
                  <div className="flex items-center justify-between border-b border-neutral-200/10 pb-3">
                    <h3 className={`text-sm font-bold flex items-center gap-2 ${activeTheme.text}`}>
                      ...
                      개인 프로젝트 포털 ({personalAppsSplit.length})
                    </h3>
                    ...
                    {personalAppsSplit.length === 0 ? (
                      ...
                      personalAppsSplit.map((app) => (
                        <a
                          ...
                          target="_blank"
                          rel="noopener noreferrer"
                          className={`p-4 rounded-xl border block transition-all duration-300 hover:shadow-md hover:border-violet-500/30 ${activeTheme.cardBg} ${activeTheme.border}`}
                        >
                          <div className="flex items-center justify-between gap-2">
                            <div className="flex items-center gap-2.5 min-w-0">
